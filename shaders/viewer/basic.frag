...
vec3 light(vec3 lpos,vec3 normal,vec3 vpos,vec3 pos,float metallic,float r){
	vec3 N = normal;
	...
	return (spec*mat.specularColor.xyz+diff*clamp((1.0-F)*(1.0f-metallic),0.0,1.0));
}
...
void main() {
	...
	const float metallic = mat.diffuseColor.w;
	const vec3  diffColor = mat.diffuseColor.rgb;

	const float r = mat.specularColor.w;
	const vec3  specColor = mat.specularColor.rgb;

	vec3 lightColor = light(lightPos,normal,view,pos,metallic,r)*radiance;
