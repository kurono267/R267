...
Mesh::Mesh(const bool storeCPU) : _isData(false),_storeCPU(storeCPU) {}
Mesh::~Mesh(){}

void Mesh::setData(const uint8_t* data,const uint64_t& size){
	if(data == nullptr)throw std::runtime_error("Mesh::setData data is null");
	unpack(data,size);
	_isData = true;
}

void Mesh::setData(const std::vector<sVertex>& vertexes,const std::vector<uint32_t>& indexes,const std::string& name){
	_name     = name;
	_vertexes = vertexes;
	_indexes  = indexes;
	_isData   = true;
...
void Mesh::createShape(){
	...
	_numIndex = _indexes.size();
	_numVertex = _vertexes.size();
...
void Mesh::unpack(const uint8_t* data,const uint64_t& size){
	...
	if(format != format::R267VB)throw std::logic_error("Wrong format VB");
	...
	std::memcpy(&format,data+offset,ui32); offset += ui32;
	if(format != format::R267IB)throw std::logic_error("Wrong format IB");
...
uint8_t* Mesh::pack(uint64_t& size){
	// Packed data
	size = 0; 
...
uint8_t* Mesh::packed(uint64_t& size){
	return pack(size);
}
...
bool Mesh::equal(const std::shared_ptr<r267::Mesh> other){
